(function(global, factory) {
  // Do not need to support modules, only the usual global scope
  global.PolymerReduxLocal = factory();
}(this, (function() {
  'use strict';
   ...
   */
  var bind = function(store, element, properties) {
    ...
    var bindings = Object.keys(properties).filter(function(name) {
      ...
      if (property.hasOwnProperty('statePath')) {
        if (!property.readOnly && property.notify) {
          console.warn('PolymerRedux: <' + String(element.constructor
              .is) + '>.' + String(name) +
    ...
    var update = function(state) {
      bindings.forEach(function(name) {
        ...
        var value = typeof statePath === 'function' ? statePath.call(
          element, state) : Polymer.Path.get(state, statePath);
      ...
      dispatch() {
        ...
        if (typeof action === 'string') {
          if (!actions || typeof actions[action] !== 'function') {
            throw new TypeError('PolymerRedux: <' + String(this.constructor
              .is) + '> has no action creator "' + action + '"');
          }
          action = actions[action].apply(this.constructor, args.slice(
            1));
        }
